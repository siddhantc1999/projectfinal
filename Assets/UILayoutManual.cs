...
namespace Yudiz.Gaming.Engine
{
    public abstract class _UILayoutManual : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            Debug.Log("Screen Resio" + currentScreenRatio);
        }
        ...
        void AutoUISetting()
        {
            foreach (UIObject t in SetObjects)
            {
                ...
                if (t.IsNotVisible)
                {
                    switch (t.visiblemode)
                    {
                        case Visiblemode.less:

                            t.ObjectToSet.gameObject.SetActive(t.Aspect < currentScreenRatio ? false : true);
                            break;

                        case Visiblemode.Greter:
                            t.ObjectToSet.gameObject.SetActive(t.Aspect > currentScreenRatio ? false : true);
                            break;
                        default:
                            break;
